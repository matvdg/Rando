...
class GpxManager {
  ...
  func estimations(for poi: Poi) -> Estimations {
    ...
    // Add elevation to POI (usefull if the POI is outside the Cagateille polyline like a summit)
    ...
    // At the contrary we don't add elevation from the user if he is outside the Cagateille polyline as the elevation from the GPS is not accurate, particularly in mountains and if the user is far away from the Cagateille (at home, looking at the app for example from Toulouse, so the elevation added would have been irrelevant)
  ...
  private func getLocations() -> [CLLocation] {
    let filepath = Bundle.main.path(forResource: "cirque-de-cagateille", ofType: "gpx")!
